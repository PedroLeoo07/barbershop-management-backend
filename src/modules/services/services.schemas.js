...
// =====================================================
// SCHEMAS DE VALIDAÇÃO PARA SERVIÇOS
// =====================================================
...
class ServiceSchemas {
  ...
  static get createService() {
    return z.object({
      ...
      const priceMax = data.price_max || data.price_min;
      return priceMax >= data.price_min;
    }, {
      message: 'Preço máximo deve ser maior ou igual ao preço mínimo',
      path: ['price_max']
    }).refine(data => {
      // Validar range de duração
      const durationMax = data.duration_max || data.duration_min;
      return durationMax >= data.duration_min;
    }, {
      message: 'Duração máxima deve ser maior ou igual à duração mínima',
      path: ['duration_max']
    }).transform(data => {
      // Set default values
      if (!data.price_max) data.price_max = data.price_min;
      if (!data.duration_max) data.duration_max = data.duration_min;
      return data;
    });
  }
